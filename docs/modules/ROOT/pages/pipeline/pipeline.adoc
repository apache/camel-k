...
If you need more control on the build pipeline or you need any customization, you are invited to use an xref:pipeline/external.adoc[External CICD pipeline] instead.

WARNING: the following documentation related to "custom tasks" is deprecated and will be removed in future releases.
...
NOTE: from version 2.11.0 onward, you will need to explicitly enable the feature setting `BUILDER_TASKS_ENABLED` environmnent variable as `true` on the operator Deployment.
