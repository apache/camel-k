...
In this section, we will describe how to use the https://github.com/apache/maven-mvnd[Maven Daemon] (mvnd) as a build strategy additional to the existing pod/routine strategy to build and deploy integrations faster using techniques known from Gradle and Takari.

mvnd starts a daemon process, which helps avoid restarting the built JVM every time, caches the compiled JIT code, and stores the class holders of the maven plugin, speeding up the build.

The mvnd daemon is started by the Maven plugin, and is configured in the `spec.build.maven.daemon` field of the IntegrationPlatform resource.

To build integrations faster using the mvnd daemon, 

- Check the IntegrationPlatform resource,

```sh
  $ kubectl get ip camel-k -o yaml
```
and configure the mvnd daemon in the `spec.build.maven.daemon` field. If the spec.build is configured to mvnd strategy, the maven daemon is automatically started when the integration is built.


- If the spec.build is not configured to use mvnd as a build strategy, then configure the spec.build.maven.daemon field to use the mvnd daemon. 

Edit th IntegrationPlatform resource, and set the strategy to `mvnd` in the `spec.build.strategy` field, eg:
...
spec:
  build:
    buildeStrategy: mvnd
```
such that the resource yaml is updated to: 
