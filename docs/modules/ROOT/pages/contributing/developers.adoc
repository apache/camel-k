...
[NOTE]
====
Integration tests by default use a https://github.com/sonatype/docker-nexus3[Nexus Repository Manager] for Maven repository mirror to speed up test runs if it is discovered as `nexus` service in `nexus` namespace.
...
You can explicitly disable the usage of Nexus mirror in e2e testing by setting environment variable: `TEST_DISABLE_NEXUS=true`
====
