...
func newCmdInstall(rootCmdOptions *RootCmdOptions) *cobra.Command {
	...
	cmd.Flags().StringVar(&impl.operatorImage, "operator-image", "", "Set the operator image used for the operator deployment and external builder pods")
...
func (o *installCmdOptions) install(_ *cobra.Command, _ []string) error {
	...
	if o.clusterSetupOnly {
		...
		if o.operatorImage != "" {
			platform.Spec.Build.OperatorImage = o.operatorImage
		}
		if o.buildStrategy != "" {
