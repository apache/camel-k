...
func (command *describeKameletCommandOptions) run(cmd *cobra.Command, args []string) error {
	...
	kamelet := kameletsv1.NewKamelet(command.Namespace, args[0])
...
func (command *describeKameletCommandOptions) describeKamelet(cmd *cobra.Command, kamelet kameletsv1.Kamelet) (string, error) {
		...
		if len(kamelet.Spec.Sources) > 0 {
			...
			for _, s := range kamelet.Spec.Sources {
				...
				w.Writef(1, "%s\t%s\t%t\t%s\t%s\n",
					itSource.Name,
					itSource.InferLanguage(),
					itSource.Compression,
					itSource.ContentRef,
					itSource.ContentKey)
			}
		}
