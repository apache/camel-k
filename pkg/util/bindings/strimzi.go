...
func (s StrimziBindingProvider) getBootstrapServers(ctx BindingContext, clusterName, namespace string) (string, error) {
	cluster, err := s.Client.KafkaV1().Kafkas(namespace).Get(ctx.Ctx, clusterName, v1.GetOptions{})
	if err != nil {
		return "", err
	}
...
func (s StrimziBindingProvider) lookupTopic(ctx BindingContext, endpoint camelv1.Endpoint) (*strimziv1.KafkaTopic, error) {
	...
	topic, err := s.Client.KafkaV1().KafkaTopics(namespace).Get(ctx.Ctx, endpoint.Ref.Name, v1.GetOptions{})
	...
	topics, err := s.Client.KafkaV1().KafkaTopics(namespace).List(ctx.Ctx, v1.ListOptions{
