...
func TestNativeHighMemoryIntegrations(t *testing.T) {
	WithNewTestNamespace(t, func(ns string) {
		...
		t.Run("java native support", func(t *testing.T) {
			...
			Eventually(IntegrationLogs(t, ns, name), TestTimeoutShort).Should(ContainSubstring("Java Magicstring!"))
			...
			t.Run("java native same should not rebuild", func(t *testing.T) {
				...
				Eventually(IntegrationConditionStatus(t, ns, name, v1.IntegrationConditionReady), TestTimeoutShort).
					Should(Equal(corev1.ConditionTrue))
				Eventually(IntegrationLogs(t, ns, name), TestTimeoutShort).Should(ContainSubstring("Java Magicstring!"))
				Eventually(IntegrationKit(t, ns, javaNativeCloneName)).Should(Equal(IntegrationKit(t, ns, javaNativeName)()))
			})

			t.Run("java native should rebuild", func(t *testing.T) {
				name := javaNative2Name
				Expect(KamelRunWithID(t, operatorID, ns, "files/Java2.java", "--name", name,
					"-t", "quarkus.build-mode=native",
					"-t", "builder.tasks-limit-memory=quarkus-native:9.5Gi",
				).Execute()).To(Succeed())
				...
				Eventually(IntegrationConditionStatus(t, ns, name, v1.IntegrationConditionReady), TestTimeoutShort).
					Should(Equal(corev1.ConditionTrue))
				Eventually(IntegrationLogs(t, ns, name), TestTimeoutShort).Should(ContainSubstring("Java Magic2string!"))
				Eventually(IntegrationKit(t, ns, javaNative2Name)).ShouldNot(Equal(IntegrationKit(t, ns, javaNativeName)()))
			})
		...
		t.Run("groovy native support", func(t *testing.T) {
			...
			Expect(KamelRunWithID(t, operatorID, ns, "files/Groovy.groovy", "--name", name,
				"-t", "quarkus.build-mode=native",
				"-t", "builder.tasks-limit-memory=quarkus-native:9.5Gi",
			).Execute()).To(Succeed())
			...
			Eventually(IntegrationLogs(t, ns, name), TestTimeoutShort).Should(ContainSubstring("Groovy Magicstring!"))
		...
		t.Run("kotlin native support", func(t *testing.T) {
			...
			Expect(KamelRunWithID(t, operatorID, ns, "files/Kotlin.kts", "--name", name,
				"-t", "quarkus.build-mode=native",
				"-t", "builder.tasks-limit-memory=quarkus-native:9.5Gi",
			).Execute()).To(Succeed())
			...
			Eventually(IntegrationLogs(t, ns, name), TestTimeoutShort).Should(ContainSubstring("Kotlin Magicstring!"))
