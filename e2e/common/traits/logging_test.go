...
func TestLoggingCategory(t *testing.T) {
	...
	WithNewTestNamespace(t, func(g *WithT, ns string) {
		...
		t.Run("Test debug log level", func(t *testing.T) {
			...
			g.Expect(KamelRunWithID(t, operatorID, ns, "files/Java.java", "--name", name, "-t", "logging.category.'org.apache.camel.impl'=DEBUG").Execute()).To(Succeed())
			g.Eventually(IntegrationPodPhase(t, ns, name), TestTimeoutLong).Should(Equal(corev1.PodRunning))
			g.Eventually(IntegrationLogs(t, ns, name), TestTimeoutShort).Should(ContainSubstring("Adding routes from builder"))
		...
		t.Run("Test multiple log levels", func(t *testing.T) {
			...
			g.Eventually(IntegrationPodPhase(t, ns, name), TestTimeoutLong).Should(Equal(corev1.PodRunning))
			g.Eventually(IntegrationLogs(t, ns, name), TestTimeoutShort).Should(ContainSubstring("Adding routes from builder"))
			g.Eventually(IntegrationLogs(t, ns, name), TestTimeoutShort).Should(ContainSubstring("Vert.x Cache configured to"))
			g.Expect(Kamel(t, "delete", "--all", "-n", ns).Execute()).Should(BeNil())
		})
	})
}
