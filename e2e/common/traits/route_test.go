...
func TestRunRoutes(t *testing.T) {
	WithNewTestNamespace(t, func(ns string) {
		...
		t.Run("Route unsecure http works", func(t *testing.T) {
			...
			// Ensure ingresses are admitted
			Eventually(RouteAdmitted(ns, integrationName), TestTimeoutLong).Should(BeTrue())
...
func addAnnotation(ns string, name string, annotations map[string]string) error {
	svc := Service(ns, name)()
	if svc == nil {
		fmt.Printf("Error getting service %s in namespace: %s.\n", name, ns)
		return nil
	}
	newSvc := svc.DeepCopy()
	newSvc.ObjectMeta.Annotations = annotations

	err := TestClient().Update(TestContext, newSvc)
	if err != nil {
		fmt.Printf("Error updating annotations in service %s in namespace: %s. Error: %s\n", name, ns, err)
		return err
	}
	return nil
}
