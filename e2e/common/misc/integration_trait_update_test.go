...
func TestTraitUpdates(t *testing.T) {
	...
	WithNewTestNamespace(t, func(ctx context.Context, g *WithT, ns string) {
		...
		t.Run("run and update trait", func(t *testing.T) {
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/yaml.yaml", "--name", name).Execute()).To(Succeed())
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/yaml.yaml", "--name", name, "-p", "hello=world").Execute()).To(Succeed())
