	...
	WithNewTestNamespace(t, func(ctx context.Context, g *WithT, ns string) {
		...
		*/
		...
		t.Run("Simple property", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/property-route.yaml", "-p", "my.message=test-property").Execute()).To(Succeed())
			...
			g.Expect(CamelK(t, ctx, "delete", "property-route", "-n", ns).Execute()).To(Succeed())
		})
		...
		*/
		...
		t.Run("Property file", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/property-file-route.yaml", "--property", "file:./files/my.properties").Execute()).To(Succeed())
			...
			g.Expect(CamelK(t, ctx, "delete", "property-file-route", "-n", ns).Execute()).To(Succeed())
		})
		...
		*/
		...
		t.Run("Property precedence", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/property-file-route.yaml", "-p", "my.key.2=universe", "-p", "file:./files/my.properties").Execute()).To(Succeed())
			g.Eventually(IntegrationPodPhase(t, ctx, ns, "property-file-route"), TestTimeoutLong).Should(Equal(corev1.PodRunning))
			g.Eventually(IntegrationConditionStatus(t, ctx, ns, "property-file-route", v1.IntegrationConditionReady), TestTimeoutShort).Should(Equal(corev1.ConditionTrue))
		...
		*/
		...
		t.Run("Property from ConfigMap", func(t *testing.T) {
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/property-route.yaml", "-p", "configmap:my-cm-test-property").Execute()).To(Succeed())
			g.Eventually(IntegrationPodPhase(t, ctx, ns, "property-route"), TestTimeoutLong).Should(Equal(corev1.PodRunning))
			g.Eventually(IntegrationConditionStatus(t, ctx, ns, "property-route", v1.IntegrationConditionReady), TestTimeoutShort).Should(Equal(corev1.ConditionTrue))
		...
		*/
		...
		t.Run("Property from ConfigMap as property file", func(t *testing.T) {
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/property-route.yaml", "-p", "configmap:my-cm-test-properties").Execute()).To(Succeed())
			g.Eventually(IntegrationPodPhase(t, ctx, ns, "property-route"), TestTimeoutLong).Should(Equal(corev1.PodRunning))
			g.Eventually(IntegrationConditionStatus(t, ctx, ns, "property-route", v1.IntegrationConditionReady), TestTimeoutShort).Should(Equal(corev1.ConditionTrue))
		...
		*/
		...
		t.Run("Property from Secret", func(t *testing.T) {
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/property-route.yaml", "-p", "secret:my-sec-test-property").Execute()).To(Succeed())
			g.Eventually(IntegrationPodPhase(t, ctx, ns, "property-route"), TestTimeoutLong).Should(Equal(corev1.PodRunning))
			g.Eventually(IntegrationConditionStatus(t, ctx, ns, "property-route", v1.IntegrationConditionReady), TestTimeoutShort).Should(Equal(corev1.ConditionTrue))
		...
		*/
		...
		t.Run("Property from Secret as property file", func(t *testing.T) {
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/property-route.yaml", "--name", "property-route-secret", "-p", "secret:my-sec-test-properties").Execute()).To(Succeed())
		...
		*/
		...
		t.Run("Property from Secret inlined", func(t *testing.T) {
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/property-secret-route.yaml", "-t", "mount.configs=secret:my-sec-inlined").Execute()).To(Succeed())
			...
			g.Expect(CamelK(t, ctx, "delete", "property-secret-route", "-n", ns).Execute()).To(Succeed())
		...
		*/
		...
		t.Run("Config configmap", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/config-configmap-route.yaml", "--config", "configmap:my-cm").Execute()).To(Succeed())
		...
		*/
		...
		t.Run("Resource configmap", func(t *testing.T) {
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/resource-configmap-route.yaml", "--resource", "configmap:my-cm").Execute()).To(Succeed())
		...
		*/
		...
		t.Run("Resource configmap with destination", func(t *testing.T) {
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/resource-configmap-location-route.yaml", "--resource", "configmap:my-cm@/tmp/app").Execute()).To(Succeed())
		...
		*/
		...
		t.Run("Resource configmap with filtered key and destination", func(t *testing.T) {
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/resource-configmap-key-location-route.yaml", "--resource", "configmap:my-cm-multi/my-configmap-key-2@/tmp/app/test.txt").Execute()).To(Succeed())
		...
		*/
		...
		t.Run("Config configmap as property file", func(t *testing.T) {
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/config-configmap-properties-route.yaml", "--config", "configmap:my-cm-properties").Execute()).To(Succeed())
		...
		*/
		...
		t.Run("Config secret", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/config-secret-route.yaml", "--config", "secret:my-sec").Execute()).To(Succeed())
		...
		*/
		...
		t.Run("Resource secret", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/resource-secret-route.yaml", "--resource", "secret:my-sec").Execute()).To(Succeed())
		...
		*/
		...
		t.Run("Secret with filtered key", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/config-secret-key-route.yaml", "--config", "secret:my-sec-multi/my-secret-key-2").Execute()).To(Succeed())
		...
		*/
		...
		t.Run("Build time property", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/build-property-route.yaml", "--build-property", "quarkus.application.name=my-super-application").Execute()).To(Succeed())
		...
		*/
		...
		t.Run("Build time property updated", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/build-property-route.yaml", "--name", "build-property-route-updated", "--build-property", "quarkus.application.name=my-super-application-updated").Execute()).To(Succeed())
		...
		*/
		...
		t.Run("Build time property file", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/build-property-file-route.yaml", "--build-property", "file:./files/quarkus.properties").Execute()).To(Succeed())
			...
			g.Expect(CamelK(t, ctx, "delete", "build-property-file-route", "-n", ns).Execute()).To(Succeed())
		})
		...
		*/
		...
		t.Run("Build time property file with precedence", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/build-property-file-route.yaml", "--name", "build-property-file-route-precedence", "--build-property", "quarkus.application.name=my-overridden-application", "--build-property", "file:./files/quarkus.properties").Execute()).To(Succeed())
			...
			g.Expect(CamelK(t, ctx, "delete", "build-property-file-route-precedence", "-n", ns).Execute()).To(Succeed())
		})
		...
		*/
		...
		t.Run("Build time property from ConfigMap", func(t *testing.T) {
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/build-property-file-route.yaml", "--build-property", "configmap:my-cm-test-build-property").Execute()).To(Succeed())
			g.Eventually(IntegrationPodPhase(t, ctx, ns, "build-property-file-route"), TestTimeoutLong).Should(Equal(corev1.PodRunning))
			g.Eventually(IntegrationConditionStatus(t, ctx, ns, "build-property-file-route", v1.IntegrationConditionReady), TestTimeoutShort).Should(Equal(corev1.ConditionTrue))
		...
		*/
		...
		t.Run("Build time property from ConfigMap as property file", func(t *testing.T) {
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/build-property-file-route.yaml", "--name", "build-property-file-route-cm", "--build-property", "configmap:my-cm-test-build-properties").Execute()).To(Succeed())
			...
			g.Expect(CamelK(t, ctx, "delete", "build-property-file-route-cm", "-n", ns).Execute()).To(Succeed())
		})
		...
		*/
		...
		t.Run("Build time property from Secret", func(t *testing.T) {
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/build-property-file-route.yaml", "--build-property", "secret:my-sec-test-build-property").Execute()).To(Succeed())
			g.Eventually(IntegrationPodPhase(t, ctx, ns, "build-property-file-route"), TestTimeoutLong).Should(Equal(corev1.PodRunning))
			g.Eventually(IntegrationConditionStatus(t, ctx, ns, "build-property-file-route", v1.IntegrationConditionReady), TestTimeoutShort).Should(Equal(corev1.ConditionTrue))
		...
		*/
		...
		t.Run("Build time property from Secret as property file", func(t *testing.T) {
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "./files/build-property-file-route.yaml", "--name", "build-property-file-route-secret", "--build-property", "secret:my-sec-test-build-properties").Execute()).To(Succeed())
			...
			g.Expect(CamelK(t, ctx, "delete", "build-property-file-route-secret", "-n", ns).Execute()).To(Succeed())
		})
