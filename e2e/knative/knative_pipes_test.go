...
func TestKnativePipes(t *testing.T) {
	WithNewTestNamespace(t, func(ctx context.Context, g *WithT, ns string) {
		...
		t.Run("Service combo", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/knative2.yaml").Execute()).To(Succeed())
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/knative3.yaml").Execute()).To(Succeed())
			...
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/knative1.yaml").Execute()).To(Succeed())
		...
		t.Run("Channel combo v1beta1", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/knativech2.yaml").Execute()).To(Succeed())
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/knativech1.yaml").Execute()).To(Succeed())
		...
		t.Run("Channel combo get to post", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/knativegetpost2.yaml").Execute()).To(Succeed())
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/knativegetpost1.yaml").Execute()).To(Succeed())
		...
		t.Run("Multi channel chain", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/knativemultihop3.yaml").Execute()).To(Succeed())
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/knativemultihop2.yaml").Execute()).To(Succeed())
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/knativemultihop1.yaml").Execute()).To(Succeed())
		...
		t.Run("Knative-service disabled", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/http_out.yaml", "-t", "knative-service.enabled=false").Execute()).To(Succeed())
		...
		t.Run("Knative-service priority", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/http_out.yaml").Execute()).To(Succeed())
			g.Eventually(IntegrationPodPhase(t, ctx, ns, "http-out"), TestTimeoutLong).Should(Equal(v1.PodRunning))
		...
		t.Run("Knative-service annotation", func(t *testing.T) {
			g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/knative2.yaml", "-t", "knative-service.annotations.'haproxy.router.openshift.io/balance'=roundrobin").Execute()).To(Succeed())
			g.Eventually(IntegrationPodPhase(t, ctx, ns, "knative2"), TestTimeoutLong).Should(Equal(v1.PodRunning))
...
func TestRunBroker(t *testing.T) {
	WithNewTestNamespaceWithKnativeBroker(t, func(ctx context.Context, g *WithT, ns string) {
		...
		g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/knativeevt1.yaml").Execute()).To(Succeed())
		g.Expect(CamelKRunWithID(t, ctx, operatorID, ns, "files/knativeevt2.yaml").Execute()).To(Succeed())
