...
const (
	...
	nexusService     = "nexus"
	nexusMavenMirror = "http://nexus.nexus/repository/maven-public/@id=nexus@mirrorOf=*"
)

func init() {
	// Enable nexus repository mirror by default for E2E testing
	disableNexus := os.Getenv("TEST_DISABLE_NEXUS")
	if disableNexus != "true" {
		svcChecked := false
		svcExists := true
