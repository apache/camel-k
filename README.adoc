...
If you need help on how to create a local development environment based on *CRC* or *Minikube*, you can follow the https://camel.apache.org/camel-k/latest/installation/installation.html[local cluster setup guide].
...
- https://camel.apache.org/camel-k/latest/installation/crc.html[Red Hat CodeReady Containers (CRC)]
...
For CRC, this means executing `oc login -u system:admin` then `kamel install --cluster-setup` only for first-time installation.
